...
addEventListener('keydown', (e) => {
    if (e.key === 'Shift') goneToShift = true
})

/* =========================
   ROOM SWITCH FUNCTIONS
========================= */
const room1 = () => {
    activeRoom = 'room1'
    ...
    chat1.classList.add('bgyellow')
    chat2.classList.remove('bgyellow')
...
const room2 = () => {
    ...
    chat2.classList.add('bgyellow')
    chat1.classList.remove('bgyellow')
...
========================= */
const createRoom = () => {
    let roomCount = Object.keys(dynamicRooms).length + 3
    let roomName = `room${roomCount}`

    // Left tab
    let chatDiv = document.createElement('div')
    chatDiv.className = 'chat3-rooms bgwhite'
    chatDiv.innerHTML = `<h2>Room ${roomCount}</h2><p>online</p>`
    leftSection.insertBefore(chatDiv, leftSection.lastElementChild)

    // Right room
    let roomDiv = document.createElement('div')
    roomDiv.className = roomName
    roomDiv.style.display = 'none'
    roomDiv.innerHTML = `Room ${roomCount} chat`
    rightSection.appendChild(roomDiv)

    dynamicRooms[roomName] = roomDiv
...
const sendMessageToRoom = () => {
    ...
    let activeRoomDiv =
        activeRoom === 'room1' ? Room1 :
        activeRoom === 'room2' ? Room2 :
        dynamicRooms[activeRoom]

    if (!activeRoomDiv) return

    let count = activeRoomDiv.childElementCount

    let msgDiv = document.createElement('div')
    msgDiv.className = count % 2 === 0 ? 'new-elem-div' : 'new-elem-div2'

    let msg = document.createElement('h3')
    msg.innerHTML = count % 2 === 0
    ...
    activeRoomDiv.appendChild(msgDiv)
    userInput.value = ''
...
/* =========================
   HELPERS
========================= */
function hideDynamicRooms() {
    Object.values(dynamicRooms).forEach(r => r.style.display = 'none')
}

function clearRoomHighlight() {
    document.querySelectorAll('.chat1,.chat2,.chat3-rooms')
        .forEach(el => el.classList.remove('bgyellow'))
}

function mobileHideLeft() {
